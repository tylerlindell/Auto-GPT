...
def parse_arguments():
    ...
    parser.add_argument('--remember', action='store_true', help='Enable Remember Mode to use previous conversation as starting context')
    ...
    if args.remember:
        print_to_console("Remember Mode: ", Fore.GREEN, "ENABLED")
...
while True:
    ...
    if not cfg.continuous_mode and next_action_count == 0:
        ...
        while True:
            ...
            try:
                console_input = json.loads(console_input)
            except json.decoder.JSONDecodeError:
                pass

            if type(console_input) is dict and "command" in console_input:
                command_name, arguments = cmd.get_command(console_input)
            elif console_input.lower() == "y":
    ...
    memory_to_add = f"Assistant Reply: {fix_and_parse_json(assistant_reply)} " \
                    f"\nResult: {fix_and_parse_json(result)} " \
                    f"\nHuman Feedback: {fix_and_parse_json(user_input)} "
